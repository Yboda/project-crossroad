...
export function createFakeEndingNarrative(runState) {
  return {
    ...
    layout: 'exploration',
    story: [
      '문을 여는 순간 따스한 햇살이 얼굴을 감싼다. 숨이 막힐 만큼 밝고, 오래 기다린 것처럼 친숙하다.',
      '그러나 그림자가 생기지 않는다. 다음 순간, 당신은 다시 시체더미의 방으로 떨어진다.',
      '빛은 당신을 비추는 것이 아니라, 당신의 기억을 태우고 있었다.',
    ],
    prompt: '당신은,',
...
export function createTrueEndingNarrative() {
  return {
    ...
    story: [
      ...
      '조각을 흡수하자 미궁의 기억이 함께 밀려든다. 너는 다시 스스로를 나눈다—잊기 위해서가 아니라, 다른 선택을 남기기 위해서.',
      '이곳을 빠져나가면 미궁도 사라지고, 미궁이 된 너 또한 사라진다.',
    ],
    prompt: '당신은,',
